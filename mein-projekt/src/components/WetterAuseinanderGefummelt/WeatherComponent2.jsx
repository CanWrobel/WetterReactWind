...
const WeatherComponent = ({ data }) => {
    ...
    const getForecastFor16 = (forecasts) => {
        return forecasts.find((f) => new Date(f.dt * 1000).getHours() === 16) || forecasts[5]; // 5. Eintrag als Fallback
    };
                    ...
                    <div key={date} className="rw-container css-1qhov81">
                        <div className="rw-container-main">
                            <div className="rw-container-left">
                                <h2 className="rw-container-header">Berlin (Charlottenburg)</h2>
                            ...
                            {forecasts.slice(0, 8).map((item) => (
                                <div key={item.dt} className="rw-forecast-day">
                                    ...
                                    <div className="rw-forecast-desc">
                                        ...
                                        {item.rain && item.rain['3h'] ? `${item.rain['3h']} mm/h` : ''}
                                    </div>

                                    <div
                                        className="rw-forecast-range"
                                        style={{
                                            border: (item.wind.speed * 3.6) < 10 ? '3px solid green' : (item.wind.speed * 3.6) < 15 ? '3px solid yellow' : 'none',
                                            fontWeight: (item.wind.speed * 3.6) < 15 ? 'bold' : 'normal',
